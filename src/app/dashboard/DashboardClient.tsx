...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    try {
      setClientInvoices(JSON.parse(localStorage.getItem(key) ?? '[]'))
    } catch {
      setClientInvoices([])
    }
  ...
  async function addCertificate() {
    ...
    await run('Certificate', async () => {
      ...
      await createCertificate.mutateAsync({
        ...
        invoice_no: certForm.invoice_no || `INV-${(subcontractors.find(s => s.id === certForm.subcontractor_id)?.name ?? 'XXX').replace(/[^A-Za-z]/g,'').slice(0,3).toUpperCase()}-${String(nextCertNo).padStart(3,'0')}`,
            ...
            <>
              ...
              <Card title="BOQ list">
                ...
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                    <thead><tr style={{ background: '#1a6b4a', color: '#fff' }}>
                      {['Structure','Code','Description','Unit','Qty','Rate','Discipline','Actions'].map(h => <th key={h} style={{ padding: '8px 10px', textAlign: 'left', fontWeight: 600 }}>{h}</th>)}
                    </tr></thead>
                    <tbody>
                      {boqItems.map((b: BoqItemWithStructure) => {
                        const s = structures.find((st) => st.id === b.structure_id)
                        ...
                        return (
                          <tr key={b.id} style={{ borderBottom: '1px solid #f0f0f0', background: isEditing ? '#f0f7f4' : 'white' }}>
                            {isEditing ? <>
                              ...
                              <td style={{ padding: '5px 6px' }}><Select value={editBoqForm.discipline ?? ''} onChange={e => setEditBoqForm({ ...editBoqForm, discipline: e.target.value })}><option value="">—</option>{['Structural','Architectural','MEP','Civil','Landscaping','Fit-Out','Facade','Infrastructure','Other'].map(d => <option key={d} value={d}>{d}</option>)}</Select></td>
                              <td style={{ padding: '5px 6px' }}><div style={{ display: 'flex', gap: 4 }}>
                                <Button onClick={async () => { await run('Update BOQ', () => updateBoqItem.mutateAsync({ id: b.id, data: { item_code: editBoqForm.item_code, description: editBoqForm.description, unit: editBoqForm.unit, boq_qty: parseFloat(editBoqForm.boq_qty)||0, client_rate: parseFloat(editBoqForm.client_rate)||0, discipline: editBoqForm.discipline||null, structure_id: editBoqForm.structure_id||null } })); setEditingBoqId(null) }} disabled={updateBoqItem.isPending}>Save</Button>
                                <Button tone="secondary" onClick={() => setEditingBoqId(null)}>Cancel</Button>
                              </div></td>
                            </> : <>
                              <td style={{ padding: '7px 10px' }}>{s ? `${s.type} — ${s.code}` : '—'}</td>
                              ...
                              <td style={{ padding: '7px 10px' }}><div style={{ display: 'flex', gap: 4 }}>
                                <Button tone="secondary" onClick={() => { setEditingBoqId(b.id); setEditBoqForm({ item_code: b.item_code, description: b.description, unit: b.unit, boq_qty: String(b.boq_qty), client_rate: String(b.client_rate ?? b.rate ?? 0), discipline: b.discipline ?? '', structure_id: b.structure_id ?? '' }) }}>Edit</Button>
                ...
                return <>
                  ...
                  <Card title={`Breakdown — ${lines.length} BOQ Items`}>
                    ...
                    {(() => {
                      ...
                      const retAmt = grossTotal * retPct / 100
                      const dpAmt = sub2?.advance_amount ?? 0
                      const dpRecoveryPct = sub2?.advance_recovery_pct ?? 0
                      const dpRecovery = grossTotal * dpRecoveryPct / 100
                      const totalPenalties = invoicePenalties.reduce((s: number, p: any) => s + (p.amount ?? 0), 0)
                      const totalAdditions = invoiceAdditions.reduce((s: number, a: any) => s + (a.amount ?? 0), 0)
                      const finalNet = grossTotal - retAmt - dpRecovery - totalPenalties + totalAdditions
                      const outstandingBalance = finalNet // will subtract paid when implemented
                      ...
                      return (
                        <div style={{ marginTop: 24, background: '#1a1a2e', borderRadius: 12, padding: 20, color: '#fff' }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                            {[
                              ['Gross Amount (from breakdown)', money(grossTotal), '+', '#66bb6a'],
                              [`Retention (${retPct}%)`, money(retAmt), '-', '#ef9a9a'],
                              [`Down Payment Recovery (${dpRecoveryPct}% of gross)${dpAmt > 0 ? ` — Contract DP: ${money(dpAmt)}` : ''}`, money(dpRecovery), '-', '#ffcc80'],
                              [`Penalties (${invoicePenalties.length} items)`, money(totalPenalties), '-', '#ef9a9a'],
                              [`Additions (${invoiceAdditions.length} items)`, money(totalAdditions), '+', '#66bb6a'],
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderTop: '2px solid rgba(255,255,255,0.3)', marginTop: 4 }}>
                              <span style={{ fontWeight: 800, fontSize: 16 }}>= NET PAYABLE</span>
                              <span style={{ fontWeight: 800, fontSize: 20, color: '#66bb6a' }}>{money(finalNet)}</span>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 14px', background: 'rgba(255,255,255,0.08)', borderRadius: 8 }}>
                              <span style={{ fontSize: 13, color: '#aaa' }}>Outstanding Balance</span>
                              <span style={{ fontWeight: 700, color: '#ffcc80', fontSize: 15 }}>{money(outstandingBalance)}</span>
                            </div>
                          </div>
                        </div>
                      )
                    ...
                    <div style={{ display: 'flex', gap: 12, marginTop: 20 }}>
                      ...
                      <Button tone="secondary" onClick={() => {
                        ...
                        const printContent = `
                          ...
                          ${(() => {
                            const sub3 = subcontractors.find(s => s.id === selectedInvoice.subcontractor_id) as any
                            const retPct2 = selectedInvoice.retention_pct ?? 5
                            const retAmt2 = grossTotal * retPct2 / 100
                            const dpRecPct2 = sub3?.advance_recovery_pct ?? 0
                            const dpRec2 = grossTotal * dpRecPct2 / 100
                            const totalPen2 = invoicePenalties.reduce((s: number, p: any) => s + (p.amount ?? 0), 0)
                            const totalAdd2 = invoiceAdditions.reduce((s: number, a: any) => s + (a.amount ?? 0), 0)
                            const finalNet2 = grossTotal - retAmt2 - dpRec2 - totalPen2 + totalAdd2
                            return `<div class="summary">
                              <div class="summary-box"><label>Gross Amount</label><span>${money(grossTotal)}</span></div>
                              <div class="summary-box"><label>Retention (${retPct2}%)</label><span style="color:#e65100">-${money(retAmt2)}</span></div>
                              <div class="summary-box"><label>DP Recovery (${dpRecPct2}%)</label><span style="color:#e65100">-${money(dpRec2)}</span></div>
                              <div class="summary-box"><label>Penalties</label><span style="color:#c62828">-${money(totalPen2)}</span></div>
                              <div class="summary-box"><label>Additions</label><span style="color:#2e7d32">+${money(totalAdd2)}</span></div>
                              <div class="summary-box" style="border:2px solid #1a6b4a"><label>NET PAYABLE</label><span>${money(finalNet2)}</span></div>
                            </div>`
                          })()}
                          </body></html>`
